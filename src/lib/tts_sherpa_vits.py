...
tts_model_names = [
    "vits-piper-en_US-libritts-high-fp16.tar.bz2",
    "vits-piper-en_US-ryan-high-fp16.tar.bz2",
    "vits-piper-en_US-ljspeech-high-fp16.tar.bz2",
    "vits-piper-en_US-lessac-high-fp16.tar.bz2",
    "vits-piper-en_GB-cori-high-fp16.tar.bz2",
    "vits-piper-en_US-glados-high-fp16.tar.bz2",
    ...
    "vits-piper-es_ES-glados-medium.tar.bz2",
...
def init_tts(asset: str = "vits-piper-en_US-ljspeech-high.tar.bz2"):
    ...
    lexicon = next(
        (file for file in files if file.suffix == ".txt" and "lexicon" in file.name),
        None,
    )
    data = next(
        (file for file in files if "espeak" in file.name and file.is_dir()), None
    )
    dict = next((file for file in files if "dict" in file.name and file.is_dir()), None)
    tokens = next(
        (file for file in files if file.suffix == ".txt" and "token" in file.name), None
    )
