...
model_presets = {
    ...
    "lmstudio-community/granite-3.1-3b-a800m-instruct-GGUF": {
        ...
        "tool_use_grammar": lambda tools: f"""
            root   ::= ("<tool_call>" "[" {gbnf_or([gbnf_sanitize(tool["function"]["name"]) for tool in tools])} "]") | (nottoolcalls .*)
            nottoolcalls ::= {gbnf_not('<tool_call>')}
        """,
        "tool_use_regex": "^<tool_call>(\\[.*\\])$",
    },
